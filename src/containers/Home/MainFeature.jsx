    ...
    return (
        <section>
            ...
            <div className="mt-10 flex items-center justify-center gap-10 sm:gap-[6.25rem] text-white text-3xl sm:text-[2.5rem] font-black">
                {feature.map(menu => (
                <NavLink to={`/${menu.name}?&province=jawa%20timur&city=malang&date=${date}`} key={menu.name} className="bg-yellow-400 w-full sm:w-48 aspect-square sm:h-[12.5rem] flex flex-col items-center justify-center rounded-2xl text-white">
